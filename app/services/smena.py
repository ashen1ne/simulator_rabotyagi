...
class SmenaService(BaseService[Smena]):
    ...
    async def generate_2_2_schedule(
        self, rabotyaga_id: int, start_date: datetime, count_months: int = 2
    ):
            ...
            for _ in range(2):
                if current_date < end_date:
                    shift = Smena(
                        ...
                        start_smena=current_date.replace(
    ...
    async def get_smeny_by_rabotyaga(self, rabotyaga_id: int) -> list[Smena]:
        ...
        stmt = (
            select(self.model)
            .where(self.model.rabotyaga_id == rabotyaga_id)
            .order_by(self.model.start_smena)
        )
