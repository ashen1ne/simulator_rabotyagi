...
@router.post(
    "/generate", response_model=list[SmenaResponse], status_code=status.HTTP_201_CREATED
)
...
@router.get(
    "/rabotyaga", response_model=list[SmenaResponse], status_code=status.HTTP_200_OK
)
...
async def open_worker_smena(
    ...
    smena = await service.open_smena(
        smena_id=smena_id, rabotyaga_id=current_rabotyaga.id
    )
...
async def close_worker_smena(
    ...
    smena = await service.close_smena(
        smena_id=smena_id, rabotyaga_id=current_rabotyaga.id
    )
...
async def get_worker_stats(
    ...
    return {
        ...
        "total_shifts_completed": len(
            [s for s in history if s.status == Status.vipolena]
        ),
        "history": history,
    }
